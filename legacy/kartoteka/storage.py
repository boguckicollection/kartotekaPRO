...
def column_capacity(box: int) -> int:
    """Return the per-column capacity for a given ``box``.

    Defaults to the global :data:`BOX_COLUMN_CAPACITY` but prefers a value
    derived from :data:`BOX_CAPACITY` and :data:`BOX_COLUMNS` when the box
    defines a custom total capacity/column layout (e.g. K100 = 1000 total with
    two columns => 500 per column).
    """

    cols = BOX_COLUMNS.get(box)
    total = BOX_CAPACITY.get(box)
    try:
        if cols and total and total % cols == 0:
            return total // cols
    except Exception:
        pass
    return BOX_COLUMN_CAPACITY
...
def location_to_index(code: str) -> int:
    ...
    if offset is None:
        return 0
    per_col = column_capacity(box)
    return offset + (column - 1) * per_col + (pos - 1)
    ...
    for row in output_data:
        ...
        for code in str(row.get("warehouse_code") or "").split(";"):
            ...
            if offset is None:
                continue
            per_col = column_capacity(box)
            idx = offset + (column - 1) * per_col + (pos - 1)
            used.add(idx)
