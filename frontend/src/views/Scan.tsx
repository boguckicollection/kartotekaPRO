...
export default function ScanView({ session, preview, loading, analyzing, status, ripple, result, selected, onPick, onFile, onFolderChange, onCsvUpload, onStartSession, onSubmit, onConfirm, rightExtras, showFileInputs = true, torchSupported=false, torchOn=false, onToggleTorch, tapFocusSupported=false, onTapToFocus, lowLight=false, qualityCommitMin=0.55, qualityLive=null, onEndSession, onForceCommit, videoRef, initStatus, zoomCaps, setZoom, onBatchScan }: Props){
  ...
  const handleCandidateSelect = async (candidate: Candidate) => {
    ...
    try {
      ...
      // 3. Force-update all data from new candidate (user selected a different card!)
      const newFormData = { 
        ...enrichedData, // Start with NEW data (override everything!)
        // Force-update critical fields from candidate to ensure they're always fresh
        // Use candidate data as fallback if detailed data is missing or empty
        name: enrichedData.name || candidate.name,
        number: (enrichedData.number && String(enrichedData.number).trim()) ? enrichedData.number : (candidate.number || ''),
      ...
      // CRITICAL: Always set Card Type to "Nie dotyczy" unless Vision API detected a special variant
      // This ensures the field is never empty
      if (!newFormData['39']) { // Typ karty (Card Type)
        newFormData['39'] = '182'; // Nie dotyczy (N/A)
      }

      // Auto-set category_id (set_id) from set symbol if available
      const setSymbol = enrichedData.set_code || candidate.set_code;
      if (setSymbol && !newFormData.set_id) {
        try {
          const mapRes = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8000'}/shoper/map_set_symbol?symbol=${encodeURIComponent(setSymbol)}`);
          if (mapRes.ok) {
            const mapData = await mapRes.json();
            if (mapData.category_id) {
              newFormData.set_id = mapData.category_id;
              console.log(`Auto-mapped set symbol ${setSymbol} to category_id ${mapData.category_id}`);
            }
          }
        } catch (err) {
          console.warn(`Failed to map set symbol ${setSymbol}:`, err);
        }
      }
      ...
      const fetchOriginalScan = async () => {
        try {
          ...
          if (mergedResult.detected) {
            ...
            // CRITICAL: Always default to "Nie dotyczy" unless Vision API detected a special variant
            if (!newFormData['39']) { // Typ karty (Card Type)
              newFormData['39'] = '182'; // Nie dotyczy (N/A)
            }
  ...
  const handleCalculatePrice = async () => {
    ...
    try {
      ...
      const data = await res.json();
      setManualPriceEdit(true); // Enable manual override mode to prevent auto-overwrite
                      ...
                      <div className="flex flex-col gap-2 relative"> 
                        {isFetchingDetails && (
                          <div className="absolute inset-0 bg-black/60 backdrop-blur-sm rounded-lg flex items-center justify-center z-10">
                            <div className="flex flex-col items-center gap-2">
                              ...
                              <span className="text-white text-sm font-medium">Pobieram szczegóły karty...</span>
                            </div>
                          </div>
                        )}
                        {scanResult.candidates.slice(0, 3).map((candidate: Candidate) => (
                          <div
                            ...
                            onClick={() => !isFetchingDetails && handleCandidateSelect(candidate)}
                          >
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="col-span-2 flex items-end gap-2">
                      ...
                      <div className="flex gap-1">
                        <button 
                          onClick={() => {
                            let num = formData.number ? String(formData.number).split('/')[0].trim() : '';
                            // Normalize: SV029 -> SV29
                            const m = num.match(/^([A-Za-z]*)(0+)([1-9][0-9]*)$/);
                            if (m) num = m[1] + m[3];
                            
                            const searchString = `${formData.name || ''} ${num}`.trim();
                            if (searchString) {
                              const url = `https://www.cardmarket.com/en/Pokemon/Products/Search?category=-1&searchString=${encodeURIComponent(searchString)}&searchMode=v1&language=1&minCondition=2`;
                              window.open(url, '_blank');
                            }
                          }}
                          className="h-10 px-3 rounded-lg bg-blue-600 text-white text-xs font-bold relative group flex items-center justify-center"
                          disabled={!formData.name && !formData.number}
                          title="Cardmarket (EN, NM)"
                        >
                          CM
                        </button>
                        <button 
                          onClick={() => {
                            ...
                            const searchString = `${formData.name || ''} ${num}`.trim();
                            if (searchString) {
                              const url = `https://www.cardmarket.com/en/Pokemon/Products/Search?category=-1&searchString=${encodeURIComponent(searchString)}&searchMode=v1&language=1&minCondition=2&isFoil=Y`;
                              window.open(url, '_blank');
                            }
                          }}
                          className="h-10 px-2 rounded-lg bg-purple-600 text-white text-xs font-bold"
                          disabled={!formData.name && !formData.number}
                          title="Holo (Foil)"
                        >
                          H
                        ...
                        <button 
                          onClick={() => {
                            ...
                            const searchString = `${formData.name || ''} ${num}`.trim();
                            if (searchString) {
                              const url = `https://www.cardmarket.com/en/Pokemon/Products/Search?category=-1&searchString=${encodeURIComponent(searchString)}&searchMode=v1&language=1&minCondition=2&isReverseHolo=Y`;
                              window.open(url, '_blank');
                            }
                          }}
                          className="h-10 px-2 rounded-lg bg-teal-600 text-white text-xs font-bold"
                          disabled={!formData.name && !formData.number}
                          title="Reverse Holo"
                        >
                          R
