          ...
          article.innerHTML = `
            <a href="${escapeHtml(cardLink)}" class="block">
                <div class="aspect-[2.5/3.5] bg-zinc-800 rounded mb-2 relative overflow-hidden">
                    ...
                    ${quantity > 1 
                        ? `<div class="absolute top-2 right-2 bg-black/60 backdrop-blur-sm px-1.5 py-0.5 rounded text-[10px] font-medium text-white border border-white/10">${quantity}×</div>` 
                        : ""
                    }
                    ${rarity 
           ...
           article.innerHTML = `
            ...
            <div class="space-y-2">
                ...
                <div class="flex items-center justify-between gap-2">
                    <button type="button" class="w-6 h-6 flex items-center justify-center rounded bg-zinc-800 text-zinc-400 hover:text-white hover:bg-zinc-700" data-action="decrease" aria-label="Decrease">−</button>
                ...
                <div class="flex gap-2">
                    ...
                    <button type="button" class="py-1 px-2 bg-zinc-800 hover:bg-red-900/30 text-zinc-400 hover:text-red-400 text-[10px] font-medium rounded" data-action="delete" data-entry-id="${entry.id}">Usuń</button>
                </div>
            </div>
          `;
      }
...
const renderSearchResults = (results, viewMode) => {
  const container = document.getElementById("card-search-results");
  if (!container) return;
  ...
  for (const item of results) {
    const article = document.createElement("article");
    const cardName = item.name || "Nieznana karta";
    const setName = item.set_name || "Nieznany dodatek";
    const price = typeof item.price === "number" ? `${item.price.toFixed(2)} PLN` : "Brak ceny";
    const rarity = item.rarity || "";
    const setCode = item.set_code || "";
    const number = item.number || "";

    if (viewMode === "grid") {
      article.className = "bg-zinc-900 border border-zinc-800 rounded-lg p-3 hover:border-zinc-700 transition-colors group flex flex-col";
      article.innerHTML = `
        <a href="${escapeHtml(item.url)}" class="block mb-3">
          <div class="aspect-[2.5/3.5] bg-zinc-800 rounded overflow-hidden">
            ${item.image_small 
              ? `<img src="${escapeHtml(item.image_small)}" alt="${escapeHtml(cardName)}" class="w-full h-full object-cover" loading="lazy" />` 
              : `<div class="flex items-center justify-center h-full text-zinc-700"><i data-lucide="image"></i></div>`
            }
          </div>
        </a>
        <div class="flex flex-col flex-grow mt-auto">
          <h3 class="text-sm font-medium text-zinc-200 truncate flex-grow" title="${escapeHtml(cardName)}">${escapeHtml(cardName)}</h3>
          <div class="flex justify-between items-center text-xs text-zinc-500 mt-1">
            <span class="truncate max-w-[70%]">${escapeHtml(setName)}</span>
            <span>#${escapeHtml(number)}</span>
          </div>
          <div class="flex justify-between items-end mt-2 pt-2 border-t border-zinc-800">
            <div class="text-sm font-semibold text-emerald-400">${escapeHtml(price)}</div>
            <button class="add-to-collection-btn" data-card-id="${item.id}" data-card-name="${escapeHtml(cardName)}">
              <i data-lucide="plus-circle" class="w-5 h-5 text-zinc-500 hover:text-indigo-400"></i>
            </button>
          </div>
        </div>
      `;
    } else {
      // List view rendering can be improved here as well
      article.innerHTML = `<div>... List view for ${cardName} ...</div>`;
    }
    container.appendChild(article);
  }
  if (typeof lucide !== "undefined") {
...
const renderPagination = (currentPage, totalPages) => {
  ...
  nextButton.innerHTML = 'Następna <i data-lucide="arrow-right" class="w-4 h-4 ml-2"></i>';
