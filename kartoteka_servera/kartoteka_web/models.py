...
class CardImageHash(SQLModel, table=True):
    """Stores perceptual hashes of card images for visual search."""
    id: Optional[int] = Field(default=None, primary_key=True)
    phash: str = Field(index=True)  # Hex string of perceptual hash
    card_record_id: Optional[int] = Field(default=None, foreign_key="cardrecord.id")
    confidence: float = Field(default=1.0)  # How sure we are this hash matches this card
    created_at: dt.datetime = Field(
        default_factory=lambda: dt.datetime.now(dt.timezone.utc)
    )
